...
// ringBuffer is a bounded byte buffer for capturing QEMU stderr. When full,
// older bytes are discarded to keep the last ringBufferSize bytes.
...
// drainTo reads from src until EOF, writing each chunk to dst.
