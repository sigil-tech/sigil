...
You are working on the `aetherd` Go daemon for the Aether OS project.
Repo: `github.com/wambozi/aether` (local path is the current working directory).
...
functionality, and is NOT wired into `cmd/aetherd/main.go`.
...
Create `internal/config/config.go` that loads `~/.config/aether/config.toml` and merges
...
- `cmd/aetherd/main.go` wired: load config before flag parsing, flags override
...
`cmd/aetherd/main.go` handles SIGINT/SIGTERM via `signal.NotifyContext` but has no
...
- `aetherctl status` response JSON includes `rss_mb` field
- The `status` socket handler in `cmd/aetherd/main.go` must populate `rss_mb`
...
Acceptance criteria:
- Background goroutine in `cmd/aetherd/main.go` calls `ntf.FlushDigest()` at the
...
- `aetherctl status` includes `next_digest_at` (RFC3339) when notifier level == 1
...
Acceptance criteria:
- `deploy/aetherd.service` — valid systemd user unit:
  ```
  [Unit]
  ...
  ```
- File lives at `deploy/aetherd.service` in the repo

---

### Issue #9 — aetherd init subcommand

One command to bootstrap everything: shell hook, config file, data dir, systemd service.

Acceptance criteria:
- `aetherd init` detects `$SHELL` and appends the correct hook source line to
  `~/.zshrc` or `~/.bashrc` (only if not already present)
- Creates `~/.config/aether/config.toml` from defaults if it does not exist
- Creates `~/.local/share/aetherd/` data directory
- On Linux: copies `deploy/aetherd.service` to `~/.config/systemd/user/aetherd.service`
  and runs `systemctl --user enable --now aetherd`
- Prints a confirmation summary of each action taken
- Implement in `cmd/aetherd/main.go` as an `init` subcommand (check `os.Args[1]`)

---

### Issue #14 — aetherctl config command

`aetherctl config` is in the CLI help text but not implemented.

Acceptance criteria:
- New socket method `"config"` registered in `cmd/aetherd/main.go`; handler returns
  the resolved config as JSON with API key / token fields masked (`***`)
- `cmd/aetherctl/main.go` `config` subcommand: sends `{"method":"config","payload":{}}`
  and prints response in `key = value` format using `text/tabwriter`

---

### Issue #21 — aetherctl purge and export (privacy commands)
...
- Each matrix job: `go build` with `-ldflags "-X main.version=${{ github.ref_name }}"`,
  produces binary named `aetherd-<os>-<arch>` and `aetherctl-<os>-<arch>`
...
- `scripts/install.sh`:
  ...
  - Downloads correct `aetherd` and `aetherctl` binaries from latest GitHub Release
  - Verifies SHA256 checksum before installing
  - Installs to `~/.local/bin/`
  - Runs `aetherd init` automatically
...
- **How to audit:** `aetherctl events --all`, `aetherctl export`
...
- Prominent one-line install command: `curl -fsSL https://raw.githubusercontent.com/wambozi/aether/main/scripts/install.sh | bash`
- `aetherctl` command reference table (all subcommands with one-line descriptions)
- Link to PRIVACY.md
...
- [ ] RSS stays under 50MB during normal operation (verified via `aetherctl status`)
...
- [ ] All 10+ aetherctl commands return correct responses via socket
- [ ] Shell hook: commands appear in `aetherctl events` within 1 second of execution
...
```bash
curl -s "https://api.github.com/repos/wambozi/aether/milestones/1" \
  -H "Authorization: token $GITHUB_TOKEN" | grep '"open_issues"'
```

If `open_issues` is 1 (only the epic #2 remains open), close the epic:
```bash
curl -s -X PATCH "https://api.github.com/repos/wambozi/aether/issues/2" \
